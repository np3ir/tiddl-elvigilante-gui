...
2. Install ffmpeg — macOS does **not** bundle it (same model as Linux): `brew install ffmpeg`. Launched from Finder the app finds Homebrew's ffmpeg automatically (`/opt/homebrew/bin` on Apple Silicon, `/usr/local/bin` on Intel). If ffmpeg is missing the app tells you and stays open — install it, restart, and try again.
